...
bool Beneficiaire::ajouter()
{
        ...
        query.prepare("insert into BENEFICIAIRES (CIN,NOM,PRENOM,AGE,ADRESSE,TYPE,NUM,DATEAJOUT,STATUS) values (:CIN, :name, :lastname, :age, :adress, :type, :num, :date, DEFAULT)");
...
void Beneficiaire::log(int x, QString username)
{QSqlQuery query;
    QString OP;
    query.prepare("insert into LOGS (DATEOP,USERNAME,OP) values (SYSDATE, :username, :OP)");
    if(x==0)
        OP="0";
    else if(x==1)
        OP="1";
...
bool Beneficiaire::is_number(const std::string& s)
{
...
bool Beneficiaire::has_any_digits(const std::string& s)
{
